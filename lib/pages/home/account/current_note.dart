...
class CurrentNotePage extends StatefulWidget {
  const CurrentNotePage({Key? key}) : super(key: key);

  @override
  State<CurrentNotePage> createState() => _CurrentNoteState();
}

class _CurrentNoteState extends State<CurrentNotePage> {
  late User user;
  List<Note> notes = [];
  ...
  Future<void> loadNotes() async {
    try {
      List<Note> loadedNotes = await getStatusNotesTeky(user, );
      setState(() {
        notes = loadedNotes;
      });
    } catch (error) {
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return GestureDetector(
                        ...
                        decoration: BoxDecoration(
                          color: Color.fromARGB(255, 46, 213, 199),
